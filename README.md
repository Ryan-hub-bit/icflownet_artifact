...
This branch contains the smallest reviewer-facing workflow for the three
...
seccomp profile for Pin.

The complete source tree is retained for inspection and reuse. Full-corpus
training and dataset regeneration remain outside the timed functional workflow
because the minimal AE data package does not contain the complete raw corpus.
...
The package contains 185 clean-test records and 20 training-smoke records.
The training sample covers all four tasks: `ret`, `jumptable`, `indirectcall`,
and `tailcall`.
...
`DATA_ROOT` to that directory instead of its parent. `DATA_ROOT/model` must
exist.
...
Run the preflight and four-record, two-GPU MTL evaluation:

```bash
python "$AE_SCRIPTS/check_setup.py" "$DATA_ROOT"
...
E1 runs entirely in the released Docker image. It does not use the host Conda
environment or require a GPU. Docker must run on an x86-64 Linux host with
Internet access, `SYS_PTRACE`, and an unconfined seccomp profile:
...
```text
overall: Hub F1=0.935525, No-Hub F1=0.927454
long_range: Hub F1=0.930946, No-Hub F1=0.924675
E2 PASS
```
...
Expected result with the released package:

```text
overall: MTL F1=0.954714, Single F1=0.935525
long_range: MTL F1=0.957179, Single F1=0.930946
E3 PASS
```
